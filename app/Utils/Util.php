...
class Util
{
    ...
    private function sendSmsViaTwilio($data)
    {
     ...
     private function sendSmsViaOoredoo($data)
    {
        $sms_settings = $data['sms_settings'];
    
       if (empty($sms_settings['ooredoo_username']) || empty($sms_settings['ooredoo_access_key']) || empty($sms_settings['ooredoo_api_url']) || empty($sms_settings['ooredoo_token'])) {
            return false;
        }
        ...
        foreach ($numbers as $number) {
            try {
                $response = $client->post($sms_settings['ooredoo_api_url'], [
                    'headers' => [
                        'Authorization' => 'Bearer ' . $sms_settings['ooredoo_token'],
                    ],
                    'form_params' => [
                        'username' => $sms_settings['ooredoo_username'],
                        'access_key' => $sms_settings['ooredoo_access_key'],
                ...
                \Log::error('Ooredoo SMS Error: ' . $e->getMessage());
            }
        }
    ...
    public function sendSms($data)
    {
        ...
        if ($sms_service == 'twilio') {
         ...
         if ($sms_service == 'ooredoo') {
            return $this->sendSmsViaOoredoo($data); // New Ooredoo handler
        }
